...
- 機能1(置き換え提案)の`SYSTEM_PROMPT`には「同じ料理名でも毎回同じ提案に偏らないように」という指示があるが、実際にAIに任せてみると、豆腐・こんにゃく麺のような「ダイエットレシピの鉄板ネタ」に偏りがちなことを実測で確認した([experiments/suggestion_diversity/check_similarity.py](check_similarity.py)参照)。
...
## 実装順序(推奨)
1. ①(土台)から着手し、実際に動かして`HISTORY_WINDOW`・`SIMILARITY_THRESHOLD`の妥当性を確認する
2. ②(クイズ)は①に相乗りする形で追加(コスト増なしなので、①と同時でもよい)
3. ③(レーティング)は、①②の運用でデータが溜まってから、一番最後に着手する
